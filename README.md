...
| 차단 목록 | `private` + setter 없음 | `blockedId` |
...
- `getSubId()`와 `setBlockedId()`는 **일부러 만들지 않았어요.** getter/setter를 안 만드는 것도 설계예요.
...
| 에필로그 | 떡밥 | 차단 목록이 하나뿐이라 부계로 우회 가능 → 다음 챕터로 |
...
2. `Main.java` 우클릭 → `Run As` → `Java Application`
